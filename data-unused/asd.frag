...
<shader language="GLSL">
  <vertex><![CDATA[
    ...
    void main() {
      ...
      vec4 tc0 = mtc;
      vec4 tc1 = mtc.xxxy + vec4(    -dx,   0,  dx, -2.0*dy);  //  A1 B1 C1
      vec4 tc2 = mtc.xxxy + vec4(    -dx,   0,  dx,     -dy);  //   A  B  C
      vec4 tc3 = mtc.xxxy + vec4(    -dx,   0,  dx,       0);  //   D  E  F
      vec4 tc4 = mtc.xxxy + vec4(    -dx,   0,  dx,      dy);  //   G  H  I
      vec4 tc5 = mtc.xxxy + vec4(    -dx,   0,  dx,  2.0*dy);  //  G5 H5 I5
      vec4 tc6 = mtc.xyyy + vec4(-2.0*dx, -dy,   0,      dy);  //  A0 D0 G0
      vec4 tc7 = mtc.xyyy + vec4( 2.0*dx, -dy,   0,      dy);  //  C4 F4 I4
    }
  ]]></vertex>
  ...
  <fragment filter="nearest"><![CDATA[
    ...
    void main() {
      ...
      vec3 A1 = texture2D(iChannel0, tc1.xw).rgb;
      vec3 B1 = texture2D(iChannel0, tc1.yw).rgb;
      vec3 C1 = texture2D(iChannel0, tc1.zw).rgb;
      vec3 A  = texture2D(iChannel0, tc2.xw).rgb;
      vec3 B  = texture2D(iChannel0, tc2.yw).rgb;
      vec3 C  = texture2D(iChannel0, tc2.zw).rgb;
      vec3 D  = texture2D(iChannel0, tc3.xw).rgb;
      vec3 E  = texture2D(iChannel0, tc3.yw).rgb;
      vec3 F  = texture2D(iChannel0, tc3.zw).rgb;
      vec3 G  = texture2D(iChannel0, tc4.xw).rgb;
      vec3 H  = texture2D(iChannel0, tc4.yw).rgb;
      vec3 I  = texture2D(iChannel0, tc4.zw).rgb;
      vec3 G5 = texture2D(iChannel0, tc5.xw).rgb;
      vec3 H5 = texture2D(iChannel0, tc5.yw).rgb;
      vec3 I5 = texture2D(iChannel0, tc5.zw).rgb;
      vec3 A0 = texture2D(iChannel0, tc6.xy).rgb;
      vec3 D0 = texture2D(iChannel0, tc6.xz).rgb;
      vec3 G0 = texture2D(iChannel0, tc6.xw).rgb;
      vec3 C4 = texture2D(iChannel0, tc7.xy).rgb;
      vec3 F4 = texture2D(iChannel0, tc7.xz).rgb;
      vec3 I4 = texture2D(iChannel0, tc7.xw).rgb;
