...
object JetExhaustRenderer {
  ...
  fun render(where: Vec2, angle: Float, size: Float, strength: Float): Collection<Renderable> {
    ...
    val aura = Sprite("jet-aura", where, Vec2(6f, 4f) * size * variation() * staticStrength, a, 0f)
    val core = Sprite("jet-core", where, Vec2(3f, .75f) * size * variation() * staticStrength2, a, 0f)
    val rings = (1..4)
      .map {
        it ->
        ...
        Sprite("jet-ring", pos + Vec2.rotated(angle) * size * variation() * 4f, Vec2.one() * s * Vec2(s * 2f, 1.5f) * variation() * 1.5f * size * staticStrength, a, 0f)
      }
    val m = 1f - FastMath.pow(1f - strength, 8f)
    val fire = Sprite("jet-fire", where, Vec2(.5f, 4f) * size * m * variation(), a, 0f)
